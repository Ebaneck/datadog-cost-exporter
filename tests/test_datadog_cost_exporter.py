...
class TestMetricExporter(unittest.TestCase):
    ...
    def test_get_api_instance(self, mock_usage_metering_api, mock_api_client):
        ...
        mock_usage_metering_api.assert_called_once_with(mock_api_client.return_value.__enter__.return_value)

        self.assertEqual(mock_api_client.call_args[1]['configuration'].api_key["appKeyAuth"], 'your_app_key')
        self.assertEqual(mock_api_client.call_args[1]['configuration'].api_key["apiKeyAuth"], 'your_api_key')
    ...
    def test_get_projected_total_cost(self, mock_usage_metering_api):
        ...
        mock_response.data = [
            MagicMock(attributes=MagicMock(
                org_name='TestOrg',
                public_id='TestPublicId',
                region='TestRegion',
                projected_total_cost=100.0,
                date=datetime.now(),
                charges=[MagicMock(charge_type='TestChargeType', cost=50.0, product_name='TestProduct')]
            ))
        ]
